...
---
## 11. Background Job
Added a FastAPI background task for non-critical usage post-processing.

The usage event is committed before the background task runs, so background processing does not block the main usage response.

The background job:
- Processes usage-event information after the request completes.
- Retries failed processing up to 3 times.
- Logs retry failures and permanently failed jobs.
- Writes successful processing records to a runtime log file.

Validation:
- Submitted a usage request with idempotency key `background-test-001`.
- Usage event ID `14` was created successfully.
- Verified `usage_background.log` contained:
